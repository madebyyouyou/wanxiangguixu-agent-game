...
旧实现让浏览器根据时间戳计算 SHA-256 token，作为防滥用签名。算法、偏移和输入都在客户端，既不能证明请求来自可信主体，也挡不住对端点的滥用调用，只徒增时钟同步和失败点。本次改造删除了 `/api/time`、浏览器签名和相应节流逻辑，删掉它不降低任何现有防护。

这里要分清三件事：Key 只存服务端，保护的是**供应商凭据的保密**，并不能阻止别人直接调用中转端点；`request-validator` 限制单次请求的角色、条数与大小，压住的是**单次成本**；真正的端点防滥用（身份鉴别、限流、配额）不在本仓库，交由部署平台，见下方 Residual risks。
